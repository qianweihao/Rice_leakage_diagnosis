# -*- coding: utf-8 -*-
"""
Step: 根据"异常/排水"新逻辑提取事件段，并加入多重过滤 - 优化版本
...
DROP_TH      = 0.3   # mm/h (原0.5，降低40%) - 关键优化：更敏感的排水检测
DROP_SUM_TH  = 2.0   # mm (原4.0，降低50%) - 关键优化：更低的累计下降要求
DRAIN_LEN_H  = 3     # 连续小时数窗长度 (原5，缩短到3小时) - 关键优化：更短的检测窗口
SPIKE_TH     = 8.0   # mm，"起始点 vs 前5时段"突变阈值 (原5.0，放宽60%)
ANOM_WINDOW  = pd.Timedelta(hours=3)  # anomaly 后 X 小时内屏蔽 drain (原5小时，缩短到3小时)
